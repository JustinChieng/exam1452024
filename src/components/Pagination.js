...
function Pagination(props) {
  const { currentPage, games, onPageChangeNext, onPageChangePrevious } = props;
  return (
    <div className="pagination">
      {/* INSTRUCTION: Add a button to go to the previous page. Disable the button if the current page is the first page. */}
      <button disabled={games.length !== 0 ? true : false} onClick={onPageChangePrevious}>Previous</button>
      ...
      <button disabled={games.length === 0 ? true : false} onClick={onPageChangeNext}>Next</button>
    </div>
  );
}
